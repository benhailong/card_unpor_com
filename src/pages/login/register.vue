<template>
  <div style="">
    ...
    <div class="login">
      <div class="login-title">卡券账号注册</div>
      <div class="up-flex">
      ...
      <div style="margin-top: 20px;width: 100%">
        ...
        <a-button type="primary" style="width: 100%;margin-bottom: 20px" size="large" @click="register">注册</a-button>
...
const sendEmailCode = () => {
  ...
  instance.$http.email.send({"toEmail": email.value, "type":"register"})
      .then((res:any) => {
        console.log(res);
        countdown.value = 60;
...
const sendSmsCode = () => {
  ...
  instance.$http.sms.send({"phone": phone.value, "type":"register"})
      .then((res:any) => {
        console.log(res);
...
const register = () => {
  ...
  if (change_tab.value==0) {
    ...
    instance.$http.users.register_phone(data).then((res:any) => {
      instance.$message.success("用户注册成功")
    ...
    instance.$http.users.register_email(data).then((res:any) => {
      instance.$message.success("用户注册成功")
      upTools.goPath("/login")
    }).catch( (err:any) => {
      // instance.$message.error("????")
